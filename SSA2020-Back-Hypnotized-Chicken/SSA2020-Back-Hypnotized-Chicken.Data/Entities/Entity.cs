...
namespace SSA2020_Back_Hypnotized_Chicken.Data.Entities
{
    public abstract class Entity<T> where T : struct
    {
		...
		public string Table
		{
			get
			{
				var tableAttribute = this.GetType().GetTypeInfo().GetCustomAttribute(typeof(TableAttribute), true) as TableAttribute;
		...
		public string DeleteSql
		{
			get
			{
				return $"DELETE FROM {Table} WHERE {KeyColumn} = {Id}";
			}
		}
